...
stan () {
   ...
   if [[ "$2" = "json" ]] || [[ "$2" = "web" ]]; then
      php -d memory_limit=-1 $TEST_PATH/src/phpstan/vendor/bin/phpstan analyse $1 -c $TEST_PATH/src/phpstan/vendor/efabrica/phpstan-config/conf/${LIB_TYPE}/level.${LEVEL}.neon --no-progress --error-format=json > /var/lib/www/tools/src/phpstan-parser/data.json
      google-chrome http://localhost:8888?path=$1 </dev/null >/dev/null 2>&1 & disown
      php -S localhost:8888 -t $TEST_PATH/src/phpstan-parser
   else
...
stan-lallte () {
   ...
   if [[ -n "$1" ]]; then
     if [[ "$2" = "json" ]] || [[ "$2" = "web" ]]; then
     	php -d memory_limit=-1 $TEST_PATH/src/phpstan-latte/vendor/bin/phpstan analyse $1 -c $TEST_PATH/src/phpstan-latte/vendor/efabrica/phpstan-config/conf/nette-app/level.${LEVEL}.neon --no-progress --error-format=json > $TEST_PATH/src/phpstan-parser/data.json
     	google-chrome http://localhost:8888?path=$1 </dev/null >/dev/null 2>&1 & disown
        php -S localhost:8888 -t $TEST_PATH/src/phpstan-parser
     else
     	php -d memory_limit=-1 /var/lib/www/tools/src/phpstan-latte/vendor/bin/phpstan analyse $1 -c /var/lib/www/tools/src/phpstan-latte/vendor/efabrica/phpstan-config/conf/nette-app/level.${LEVEL}.neon --no-progress
     fi
   else
...
stan-latte app/ console 8 #stan level 8, folder "app", response "console"
stan-latte #default stan level 8, default folder "app", default response "console"
```
